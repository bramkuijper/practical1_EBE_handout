...
\DocumentationTok{\#\# 2           2       0.1               0             0}
\DocumentationTok{\#\# 3           3       0.0               0             0}
...
We will use the same payoff matrix as before (see \protect\hyperlink{tab:payoffHD}{the table in chapter 2}). However, what changes are the interaction probabilities between the individuals. Let us recall the previous fitness expressions \(W_{H}\) and \(W_{D}\) for Hawks and Doves respectively:
\begin{align}
W_{H} &= w_{0} + p_{t} \frac{v-c}{2} + \left(1 - p_{t} \right) v \label{eq:wH} \\
W_{D} &= w_{0} + p_{t} (0) + \left(1 - p_{t} \right) \frac{v}{2} \label{eq:wD}\\
\end{align}

\hypertarget{exercisee}{%
\section{Exercisee}\label{exercisee}}

Go over eqns. (\eqref{eq:wH},\eqref{eq:wD}), consult table (\eqref{eq:payoffHD}) go over the terms and check whether you understand them
